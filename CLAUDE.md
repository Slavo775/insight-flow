...
Insight Flow is a CLI workbench for AI-assisted task lifecycle management. The `insight-flow` binary owns task state (JSON files under `insightFlow/workTasks/` (legacy: `workTasks/`)) and serves a single server-rendered dashboard. There is no separate React frontend — the dashboard is generated by the CLI's HTTP server in `packages/taskflow/src/dashboard/server/`.
