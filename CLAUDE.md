...
Insight Flow is a CLI workbench for AI-assisted task lifecycle management. The `insight-flow` binary owns task state (JSON files under `insightFlow/workTasks/` (legacy: `workTasks/`)) and serves a **React + Vite** dashboard: the CLI's HTTP server in `packages/taskflow/src/dashboard/server/` serves the built React app (`packages/taskflow/src/dashboard/client/`) and its JSON APIs.
...
- **`src/dashboard/server/`** — the Node HTTP server (`index.ts`) that serves the built React dashboard and its data: the app at `/`, JSON at `/api/work-tasks/*` (hydrating per-task `reviews.json` / `incidents.json` side files into shard responses), activity at `/api/activity`, and native **SSE** for live updates.
- **`src/dashboard/client/`** — the React + Vite dashboard app (`main.tsx`, `App.tsx`, `react-router-dom`, `styled-components`, `@xyflow/react` flow editor; Kanban / timeline / detail / module + agent browser). Built by `vite build` and served by the server above; live updates stream over SSE (`useDashboardStream.ts`).
...
**Storage layout (`insightFlow/workTasks/`; pre-N101 projects: `workTasks/` until `insight-flow migrate-layout` is run)**
