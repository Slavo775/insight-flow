...
| `update` | Update the global install (`npm i -g insight-flow@latest`); then lists hub-registered projects and points at `/task-release-rollout` for per-project bumps. Global-only — never mutates projects. |
| `help` | Show help text. |
