# N23 — Architecture diagrams: agents, server, notifications, activity
...
insight-flow has four distinct conceptual subsystems (agent lifecycle, server federation, notification service, activity/milestone logging) that live in code but have never been visualised. New contributors and users struggle to understand how the pieces connect. We need four architecture diagrams — one per subsystem — with Gemini-ready prompts stored here so they can be regenerated at any time.

## Goal

1. Produce a self-contained Gemini prompt for **Diagram 1 — Agent lifecycle & roles**.
2. Produce a self-contained Gemini prompt for **Diagram 2 — Server federation (master ↔ project servers ↔ iframe UI)**.
3. Produce a self-contained Gemini prompt for **Diagram 3 — Notification service (hooks, OS notifications, browser push)**.
4. Produce a self-contained Gemini prompt for **Diagram 4 — Activity engine, milestones, and logging pipeline**.
5. All four prompts live in this TASK.md under the "Gemini Prompts" section so they are versioned with the repo.

## Scope

### In scope

- Writing the four diagram prompts below.
- Each prompt must be copy-pasteable into Gemini (or any multimodal LLM) and produce a clean architecture diagram without additional context.

### Out of scope

- Generating actual image files.
- Changing any source code.

## Implementation plan

1. **Write prompts** — fill in the "Gemini Prompts" section below.
2. **Review accuracy** — verify each prompt matches the actual code (`packages/taskflow/src/server/index.ts`, `activity.ts`, `activity-hook.ts`, `notify-hook.ts`, `init/index.ts`).
3. **Commit** — push TASK.md via `/task-git`.

## Verification

- Each prompt, when pasted into Gemini, produces a diagram that a developer unfamiliar with the codebase could use to navigate it.

## Notes

- Related tasks: N20 (master server), N21 (activity feed / milestones).
- Mermaid flowchart syntax is preferred in the prompts — Gemini renders it inline.
...
### Diagram 4 — Activity engine, milestones, and logging pipeline
