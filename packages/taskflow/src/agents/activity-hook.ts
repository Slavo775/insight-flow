 ...
 *   3 — N94 lifecycle hooks are registry modules (`activity/…`) installed via
 *       the artifact emitter's managed manifest; script content is canonical
...
export function installLifecycleHooks(
  ...
  // `force` is obsolete since N94: the emitter rewrites whenever the on-disk
  ...
  return {
    hooksWritten: reports.filter(
      (r) =>
        r.target.startsWith(".claude/hooks/") && (r.action === "created" || r.action === "updated"),
    ).length,
    settingsUpdated: reports.some(
      (r) => r.target === ".claude/settings.json" && r.action !== "unchanged",
    ),
  };
}
