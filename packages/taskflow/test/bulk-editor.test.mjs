...
const { buildBulkInitArgs } = await import(
...
test("config.editor=cursor → bare init scaffolds cursor, not claude", () => {
  ...
  try {
    ...
    assert.ok(
      existsSync(resolve(dir, ".cursor/skills")),
      "should scaffold .cursor from config.editor",
    );
...
test("--editor flag overrides config.editor", () => {
  const dir = projectWith({ editor: "cursor" });
  try {
    execFileSync(process.execPath, [CLI, "init", "--editor", "claude"], {
      cwd: dir,
      encoding: "utf-8",
    });
...
test("--editor is persisted into a freshly created config", () => {
  ...
  try {
    execFileSync(process.execPath, [CLI, "init", "--editor", "all"], {
      cwd: dir,
      encoding: "utf-8",
    });
...
test("bulk-init builds per-project init args incl. --editor passthrough", () => {
  ...
  assert.deepEqual(buildBulkInitArgs({ _: [], force: true, examples: true, editor: "cursor" }), [
    "init",
    "--force",
    "--examples",
    "--editor",
